...
public class Emetteur {
    public static void main(String[] args) {
        try {
            ...
            if (args.length != 3) {
                System.err.println("Utilisation : java Transmitter <adresse_ip_recepteur> <port_recepteur> <chemin_fichier>");
                return;
            }
            ...
            if (sendReset) {
                ...
                Packet rstPacket = new Packet(sequenceNumber++, new byte[0], false, false, false, true); // Flag RST activé
                byte[] serializedRst = Packet.serialize(rstPacket);
                DatagramPacket rstDatagram = new DatagramPacket(serializedRst, serializedRst.length, recepteurAddress, recepteurPort);
            ...
            for (int i = 0; i < fileData.length; i += 512) {
                ...
                DatagramPacket datagram = new DatagramPacket(serializedPacket, serializedPacket.length, recepteurAddress, recepteurPort);
            ...
            DatagramPacket finDatagram = new DatagramPacket(serializedFin, serializedFin.length, recepteurAddress, recepteurPort);
